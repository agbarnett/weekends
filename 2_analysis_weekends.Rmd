...
to.plot.star = mutate(to.plot.star, 
                      Europe = as.numeric(country %in% c('Denmark','France','Netherlands','UK','Sweden')),
...
to.table = select(DIC, -journal, -outcome, -data) %>% # information already in title
  arrange(DIC)
...
The seasonal pattern in submissions peaks in January with an odds ratio of 1.1. The country-specific estimates show a number of countries close to the centre, meaning they have little seasonal pattern. The strongest seasonal patterns are in Israel, Norway, Finland and New Zealand.
...
There was no difference in the overall probability of weekend submissions over time.
...
There is a very large difference between countries in the probability of weekend submissions to BMJ Open. China and Saudi Arabia both also have relatively high probabilities of weekend submissions.
...
There is a tie for the best model between a seasonal model with a country-specific slope and common slope. We choose the common slope because this is the simpler model.
...
# Statistical models - evenings and late night submissions

## BMJ late night submissions

This section examines journal submissions on evenings and late nights to the BMJ.

### Best fitting model

First we examine the deviance information criterion (DIC) to find the best model

```{r best.bmj.latenight.submissions}
DIC = NULL
which.outcome = 'latenight'
which.data.text = 'submissions' # data with windows
which.data.text.update = 'submissions.latenight'
which.data = get(which.data.text.update)
which.journal  = 'BMJ'
for (type in c('slope','intercept')){
  for (season in c(TRUE, FALSE)){
...
The best model has a country-specific season but common slope, hence we show the results for this model below. This means there is no plot of the country-specific slope for this outcome.

### Table of parameter estimates for best model

```{r best.bmj.latenight.submission.results}
# output table
to.table = tidy.table(ests)
pander(to.table, digits=3)
```

There was a very small increase in the overall probability of late night submissions over time.

### Plot of country-specific intercepts and 95% credible intervals for late night submissions to BMJ

```{r plot.intercepts.bmj.latenight.submissions}
source('2_plot_intercepts.R')
iplot
```

There is a large variability between countries in the probability of late night submissions. Denmark had by far the lowest probability and China had easily the largest. Two other countries with a high probability were Taiwan and Japan.

### Plot of seasonal estimates for late night submissions to BMJ

```{r plot.season.bmj.latenight.submissions, fig.width=13, fig.height=6.5}
source('2_plot_season.R')
grid.arrange(mplot, seasonplot, nrow = 1)
```

There is a strong seasonal pattern to late night submissions with a peak in the mean odds ratio close to 1.2 in late June. The country-specific seasonal pattern shows that almost every country has a peak in the middle six months of the year. Switzerland has no seasonal pattern in late night submissions.

## BMJ Open late night submissions

This section examines journal submissions on evenings and late nights to the BMJ Open.

### Best fitting model

First we examine the deviance information criterion (DIC) to find the best model

```{r best.bmjopen.latenight.submissions}
DIC = NULL
which.outcome = 'latenight'
which.data.text = 'submissions' # data with windows
which.data.text.update = 'submissions.latenight'
...
### Table of parameter estimates for best model for late night submissions to BMJ Open

```{r best.bmjopen.latenight.submission.results}
# output table
to.table = tidy.table(ests)
pander(to.table, digits=3)
```

There was no large difference in the overall probability of late night submissions over time.

### Plot of country-specific intercepts and 95% credible intervals for late night submissions to BMJ Open

```{r plot.intercepts.bmjopen.latenight.submissions}
source('2_plot_intercepts.R')
iplot
```

Denmark and New Zealand have the two lowest probabilities of late night submissions. The two highest probabilities are in China and Iran.

### Plot of country-specific slopes and 95% credible intervals for late night submissions to BMJ Open

```{r plot.slopes.bmjopen.latenight.submissions}
source('2_plot_slopes.R')
splot
```

There was relatively little variability in the slopes between countries. Ethiopia had a decline over time in late night submissions, whilst China had an increase.

### Plot of seasonal estimates for late night submissions to BMJ Open

```{r plot.season.bmjopen.latenight.submissions, fig.width=13, fig.height=6.5}
source('2_plot_season.R')
grid.arrange(mplot, seasonplot, nrow = 1)
```

Late night submissions are strongly seasonal with a peak in late July. The peak also occurs in Southern hemisphere countries (Australia and Brazil). This may be because of the academic year rather than any seasonal change (e.g., daylight hours), with the northern hemisphere academic calander being the most important set of dates that even southern hemisphere countries follow.

# Statistical models - evenings and late night reviews

## BMJ late night reviews

This section examines journal reviews on the weekend to the BMJ.

### Best fitting model for BMJ late night reviews

First we examine the deviance information criterion (DIC) to find the best model

```{r best.bmj.latenight.reviews}
DIC = NULL
which.outcome = 'latenight'
which.data.text = 'reviews' # data with windows
which.data.text.update = 'reviews.latenight'
which.data = get(which.data.text.update)
which.journal  = 'BMJ'
for (type in c('slope','intercept')){
  for (season in c(TRUE, FALSE)){
...
The best model has a country-specific slope and season, hence we show the results for that model below.

### Table of parameter estimates for best model for late night reviews to BMJ

```{r best.bmj.latenight.reviews.results}
# output table
to.table = tidy.table(ests)
pander(to.table, digits=3)
```

There was no change over time in the probability of late night reviews.

### Plot of country-specific intercepts and 95% credible intervals for late night reviews to BMJ

```{r plot.intercepts.bmj.latenight.reviews}
source('2_plot_intercepts.R')
iplot
```

There was a large variability in the probability of late night reviews. Australia had the lowest probability and Austria the highest.

### Plot of country-specific slopes and 95% credible intervals for late night reviews to BMJ

```{r plot.slopes.bmj.latenight.reviews}
source('2_plot_slopes.R')
splot
```

The dotted vertical line shows the average overall change, which is close to zero. Finland had an increase over time in the probability of late night reviews.

### Plot of seasonal estimates for late night reviews to BMJ 

```{r plot.season.bmj.latenight.reviews, fig.width=13, fig.height=6.5}
source('2_plot_season.R')
grid.arrange(mplot, seasonplot, nrow = 1)
```

There is a strong seasonal pattern in late night reviews with a peak in odds of over 1.2 in June. All countries have their peak between May and August, with the strongest patterns in Australia and Finland.

## BMJ Open late night reviews

This section examines journal reviews on the weekend to the BMJ.

### Best fitting model for BMJ Open late night reviews

First we examine the deviance information criterion (DIC) to find the best model

```{r best.bmjopen.latenight.reviews}
DIC = NULL
which.outcome = 'latenight'
which.data.text = 'reviews' # data with windows
which.data.text.update = 'reviews.latenight'
...
### Table of parameter estimates for best model for late night reviews to BMJ Open

```{r best.bmjopen.latenight.reviews.results}
# output table
to.table = tidy.table(ests)
pander(to.table, digits=3)
```

There was little difference in the probability of late night reviews over time.

### Plot of country-specific intercepts and 95% credible intervals for late night reviews to BMJ Open

```{r plot.intercepts.bmjopen.latenight.reviews, fig.width=5, fig.height=6}
source('2_plot_intercepts.R')
iplot
```

There is a wide variation between countries in the probability of late night reviews. Portugal and Greece have the highest probability. South Africa has by far the lowest probability. South Africa has sometimes experienced widespread electricity blackouts. 

### Plot of country-specific slopes and 95% credible intervals for late night reviews to BMJ Open

```{r plot.slopes.bmjopen.latenight.reviews, fig.width=5, fig.height=6}
source('2_plot_slopes.R')
splot
```

The strongest decrease over time in late night reviews was in Germany.

### Plot of seasonal estimates for late night reviews to BMJ Open 

```{r plot.season.bmjopen.latenight.reviews, fig.width=13, fig.height=6.5}
source('2_plot_season.R')
grid.arrange(mplot, seasonplot, nrow = 1)
```

There was a strong seasonal pattern in late night reviews with a peak odds ratio of over 1.1 in July. The peak for most countries is between May and September, with the strongest average seasonal pattern in Belgium.
...
# write out filename
jags.filename.results = 'JAGS.holiday.Reviews.BMJ.intercept.NoSeason.results.RData'
load(paste('Z:/weekend/', jags.filename.results, sep='')) # from process.jags.R 
# output table
to.table = tidy.table(ests)
pander(to.table, digits=3)
...
# write out filename
jags.filename.results = 'JAGS.holiday.Reviews.BMJ Open.intercept.NoSeason.results.RData'
load(paste('Z:/weekend/', jags.filename.results, sep='')) # from process.jags.R 
# output table
to.table = tidy.table(ests)
pander(to.table, digits=3)
...
The blue shaded area shows the working hours.
...
plot = ggplot(data=for.plot, aes(x=hour, y=perc, col=factor(journal)))+
  geom_rect(xmin=6.5, xmax=17.5, ymin=-Inf, ymax=Inf, fill=grey(0.9), alpha=0.2, inherit.aes=FALSE)+ # in hours period
  ...
  geom_point()+
#  geom_bar(stat='identity')+ not so good
...
plot
jpeg('figures/timeday.jpg', width=6, height=4, units='in', res=400, quality=100)
print(plot)
invisible(dev.off())
```
...
tile = ggplot(data=to.plot, aes(x=xaxis, y=k, fill=median))+
  ...
  xlab('')+
  ylab('')+
  scale_y_continuous(breaks=1:12, labels=j.labels, expand = c(0,0))+
  ...
  theme(text = element_text(size=13),
    ...
    plot.margin=unit(c(0.7,0,1,0),"cm")) + # increase margin = top, then right, bottom and left
  # add vertical lines
  geom_hline(yintercept = c(4.5,8.5)) + # Add vertical lines to tile plot to split holidays, late nights, and weekends
  # axis label outside the plot
  geom_text(y=1.5, x=-0.5, label='Submissions', size=4.5, fontface='plain')+
  geom_text(y=3.5, x=-0.5, label='Reviews', size=4.5, fontface='plain')+
  geom_text(y=5.5, x=-0.5, label='Submissions', size=4.5, fontface='plain')+
  geom_text(y=7.5, x=-0.5, label='Reviews', size=4.5, fontface='plain')+
  geom_text(y=9.5, x=-0.5, label='Submissions', size=4.5, fontface='plain')+
  geom_text(y=11.5, x=-0.5, label='Reviews', size=4.5, fontface='plain')+
  #
  geom_text(y=2.5, x=-1, label='Weekends', size=4.5, fontface='plain')+
  geom_text(y=6.5, x=-1, label='Late nights', size=4.5, fontface='plain')+
  geom_text(y=10.5, x=-1, label='Holidays', size=4.5, fontface='plain')+
  # repeat at top
  geom_text(y=2.5, x=17, label='Weekends', size=4.5, fontface='plain')+
  geom_text(y=6.5, x=17, label='Late nights', size=4.5, fontface='plain')+
  geom_text(y=10.5, x=17, label='Holidays', size=4.5, fontface='plain')
tile
jpeg('figures/tile.jpg', width=10, height=8, units='in', res=400, quality = 100)
print(tile)
invisible(dev.off())
```
...
Three Scandinavian countries (Norway, Finland and Denmark) had some of the lowest probabilities. Sweden appeared relatively high, but all it's estimates were still in the lower half of countries.
...
The plot below shows the overall probabilities of reviews and submissions on weekends and late nights. These are the overall averages across all countries together with the 95% credible intervals.

```{r mean.summary, fig.width=6, fig.height=4.5}
to.mean.plot.final = mutate(to.mean.plot,
            data = ifelse(data %in% c('reviews','Reviews'), 'Reviews', 'Submissions'),
            x = as.numeric(journal=='BMJ Open') + as.numeric(data=='Reviews')/5, # x-axis with jitter
            outcomen = 1,
            outcomen = ifelse(outcome=='latenight', 2, outcomen),
            outcomen = ifelse(outcome=='holiday', 3, outcomen),
            panel = factor(outcomen, levels=1:3, labels=c('Weekend','Late night','Holiday')))
...
Late nights were much more common than weekends.

There was a higher probability of reviews on the weekend compared with submissions. 

The only clear difference in journals was that late night reviews and submissions were more likely at BMJ than BMJ Open. 
...
to.table = mutate(to.mean.plot.final) %>%
  ...
  select(Outcome, Type, Journal, Mean, CI)
...
to.table = mutate(table.time, 
  ...
  select(outcome, journal, data, var, res) %>%
  rename(`Mean CI` = res) %>%
  filter(var=='ratio') %>% select(-var) # Just ratio
#  rename(type=data) %>% # for ratio and difference
#  tidyr::spread(var, res) #
