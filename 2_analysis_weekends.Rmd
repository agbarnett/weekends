...
plot = ggplot(data=for.plot, aes(x=hour, y=perc, col=factor(journal)))+
  geom_rect(xmin=7, xmax=17.99, ymin=-Inf, ymax=Inf, fill=grey(0.9), alpha=0.2, inherit.aes=FALSE)+ # in hours period
...
```{r tile.plot, fig.width=12, fig.height=10}
...
tile = ggplot(data=to.plot, aes(x=xaxis, y=k, fill=median))+
  ...
  geom_text(y=9.5, x=-0.5, label='Submissions', size=6, fontface='plain')+
  geom_text(y=11.5, x=-0.5, label='Reviews', size=6, fontface='plain')+
  #
  geom_text(y=2.5, x=-1, label='Weekends', size=6, fontface='plain')+
  geom_text(y=6.5, x=-1, label='Late nights', size=6, fontface='plain')+
  geom_text(y=10.5, x=-1, label='Holidays', size=6, fontface='plain')+
  # repeat at top
  geom_text(y=2.5, x=17, label='Weekends', size=6, fontface='plain')+
  geom_text(y=6.5, x=17, label='Late nights', size=6, fontface='plain')+
  geom_text(y=10.5, x=17, label='Holidays', size=6, fontface='plain')
tile
jpeg('figures/BarnettFig3.jpg', width=12, height=10, units='in', res=400, quality = 100) # bigger size for BMJ
print(tile)
invisible(dev.off())
```
