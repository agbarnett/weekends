...
to.plot = bind_rows(both.results.submissions, both.results.reviews) %>%
  mutate(panel = factor(panel, levels=1:2, labels=c('Manuscripts','Peer reviews')))
# scatter plot
splot = ggplot(data=to.plot, aes(x=phase.time, y=exp(amp), label=country))+
  ...
  facet_wrap(~panel)
# for journal
jpeg('figures/BarnettFig4.jpg', width=10, height=5, units='in', res=400, quality=100)
print(splot)
invisible(dev.off())
```
...
for.plot = mutate(both, hour = floor(local.hour),
                  typenum = ifelse(type=='Submission', 1, 2),
                  typenum = factor(typenum, levels=1:2, labels=c('Manuscripts','Peer reviews'))) %>% # change labels and put in right order
  group_by(type, typenum, journal, hour) %>%
  summarise(count = n()) %>% 
  group_by(type, typenum, journal) %>%
...
#  geom_rect(xmin=7, xmax=17.99, ymin=-Inf, ymax=Inf, fill=grey(0.9), alpha=0.2, inherit.aes=FALSE)+ # in hours period ; no longer shade in new version
  ...
  xlab('Hour of day')+
  ylab('Percent')+
  facet_wrap(~typenum, scale='free_y')
plot
# for journal
jpeg('figures/BarnettFig3.jpg', width=7.8, height=5.2, units='in', res=400, quality=100)
print(plot)
invisible(dev.off())
```
...
for (which.data.text in c('submission','reviewer')){
  ...
  # get averages per country
  source('2_plot_intercepts.R')
  save.for.later[[k]] = iplot # stored for later multi-panel plot  
...
# for later multi-panel plot
holidays.manuscripts.bmj = save.for.later[[1]]
holidays.manuscripts.bmjopen = save.for.later[[2]]
holidays.reviews.bmj = save.for.later[[3]]
holidays.reviews.bmjopen = save.for.later[[4]]
```
...
tile.weekend = ggplot(data=to.plot, aes(x=xaxis, y=k, fill=mean, label=label))+
  ...
  geom_text(y=1.5, x=-0.5, label='Manuscripts', size=6, fontface='plain')+
  geom_text(y=3.5, x=-0.5, label='Peer reviews', size=6, fontface='plain')
tile.weekend
```
...
# export
jpeg('figures/BarnettFig2.jpg', width=12, height=10, units='in', res=400, quality = 100) # bigger size for BMJ
...
tile = ggplot(data=to.plot, aes(x=xaxis, y=k, fill=median))+
  ...
  geom_text(y=1.5, x=-0.5, label='Manuscripts', size=6, fontface='plain')+
  geom_text(y=3.5, x=-0.5, label='Peer reviews', size=6, fontface='plain')+
  geom_text(y=5.5, x=-0.5, label='Manuscripts', size=6, fontface='plain')+
  geom_text(y=7.5, x=-0.5, label='Peer reviews', size=6, fontface='plain')+
  #
...
# Four by four panel for forest plots

## Weekends

```{r four.by.four.forest.weekends, fig.width=8, fig.height=8}
# add titles
best.plot.bmjopen.weekend.manuscripts = best.plot.bmjopen.weekend.manuscripts + ggtitle('BMJ Open, Manuscripts')
best.plot.bmj.weekend.manuscripts = best.plot.bmj.weekend.manuscripts + ggtitle('BMJ, Manuscripts')
best.plot.bmjopen.weekend.reviews = best.plot.bmjopen.weekend.reviews + ggtitle('BMJ Open, Peer reviews')
best.plot.bmj.weekend.reviews = best.plot.bmj.weekend.reviews + ggtitle('BMJ, Peer reviews')
# export
jpeg('figures/BarnettFig.jpg', width=8, height=8, units='in', res=300, quality=100)
grid.arrange(best.plot.bmj.weekend.manuscripts, best.plot.bmjopen.weekend.manuscripts, best.plot.bmj.weekend.reviews, best.plot.bmjopen.weekend.reviews, ncol=2, nrow=2)
invisible(dev.off())
grid.arrange(best.plot.bmj.weekend.manuscripts, best.plot.bmjopen.weekend.manuscripts, best.plot.bmj.weekend.reviews, best.plot.bmjopen.weekend.reviews, ncol=2, nrow=2)
```
...
```{r four.by.four.forest.holidays, fig.width=8, fig.height=8}
# add titles
holidays.manuscripts.bmjopen = holidays.manuscripts.bmjopen + ggtitle('BMJ Open, manuscripts')
holidays.manuscripts.bmj = holidays.manuscripts.bmj + ggtitle('BMJ, manuscripts')
holidays.reviews.bmjopen = holidays.reviews.bmjopen + ggtitle('BMJ Open, Peer reviews')
holidays.reviews.bmj = holidays.reviews.bmj + ggtitle('BMJ, Peer reviews')
# export
jpeg('figures/BarnettFigHolidays.jpg', width=8, height=8, units='in', res=300, quality=100)
grid.arrange(holidays.manuscripts.bmj, holidays.manuscripts.bmjopen, holidays.reviews.bmj, holidays.reviews.bmjopen, ncol=2, nrow=2)
invisible(dev.off())
grid.arrange(holidays.manuscripts.bmj, holidays.manuscripts.bmjopen, holidays.reviews.bmj, holidays.reviews.bmjopen, ncol=2, nrow=2)
```

## Double panel plot for journal

Weekend and holiday manuscript submissions to _The BMJ_.

```{r, fig.width=8, fig.height=4.5}
best.plot.bmj.weekend.manuscripts = best.plot.bmj.weekend.manuscripts + ggtitle('Weekends')
holidays.manuscripts.bmj = holidays.manuscripts.bmj + ggtitle('Holidays')
# export to journal for best model for weekend submissions (example forest plot)
jpeg('figures/BarnettFig1.jpg', width=11, height=4.25, units='in', res=400, quality = 100) # bigger size for BMJ
grid.arrange(best.plot.bmj.weekend.manuscripts, holidays.manuscripts.bmj, ncol=2, nrow=1)
invisible(dev.off())
grid.arrange(best.plot.bmj.weekend.manuscripts, holidays.manuscripts.bmj, ncol=2, nrow=1)

# export to Word tables
export.weekends = select(best.plot.bmj.weekend.manuscripts$data, xaxis, Country, mean, lower, upper) %>%
mutate(Country = as.character(Country),
       Country = ifelse(is.na(Country), 'Overall', Country ))
export.hols = select(holidays.manuscripts.bmj$data, xaxis, Country, mean, lower, upper) %>%
mutate(Country = as.character(Country),
       Country = ifelse(is.na(Country), 'Overall', Country ))

#
e.file = 'export.tables.txt'
out = file(e.file , 'w')
cat('Weekends\n', file=out)
for (k in 1:nrow(export.weekends)){
  cat(export.weekends$xaxis[k], export.weekends$Country[k], export.weekends$mean[k], export.weekends$lower[k], export.weekends$upper[k], '\n', sep='&', file=out)
}
cat('\nHolidays\n', file=out)
for (k in 1:nrow(export.hols)){
  cat(export.hols$xaxis[k], export.hols$Country[k], export.hols$mean[k], export.hols$lower[k], export.hols$upper[k], '\n', sep='&', file=out)
}
close(out)
